...
def predict_preferred_language_by_city(k_values, cities):
    ...
    for k in k_values:
        ...
        for a in cities:
            num_correct = knn_classify(k,cities,a[0])
            predicted_value.append((a[0],num_correct))
        my_list = [(a,b) for (a,b) in predicted_value for (c,d) in cities  if ((a==c) and (b==d))]
        num_correct = len(my_list)
        print(k, "neighbor[s]:", num_correct, "correct out of", len(cities))
